...
export default async function AdminProblemDetailPage({
  ...
  const vq = selected ? `?v=${selected.version}` : '';
          ...
          {listing?.error ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] border border-border-subtle-01">
              ...
              <div className="bg-layer-01 border-b md:border-b-0 md:border-r border-border-subtle-01 max-h-[600px] overflow-auto">
                <div className="px-05 py-03 border-b border-border-subtle-01 cds-helper-01 text-text-secondary">
                  스캐폴드 파일 {listing?.entries.filter((e) => !e.isDir).length ?? 0}개
                </div>
                <ul>
                  {listing?.entries.map((e) =>
                    e.isDir ? (
                      <li key={e.path} className="px-05 py-02 cds-code-01 text-text-secondary flex items-center gap-02">
                      ...
                      <li key={e.path}>
                        <NextLink
                          href={`${base}${vq}&file=${encodeURIComponent(e.path)}`}
                          className={`px-05 py-02 cds-code-01 flex items-center justify-between gap-03 hover:bg-layer-hover-01 ${
                            e.path === filePath ? 'bg-layer-hover-01 text-text-primary' : 'text-link-primary'
                          }`}
                        >
                          ...
                          <span className="text-text-secondary shrink-0">{fmtBytes(e.size)}</span>
                        </NextLink>
                      </li>
                    ),
                  )}
              ...
              {/* 우: 파일 내용 */}
              <div className="bg-layer-02 min-h-[320px] max-h-[600px] overflow-auto">
                {!preview ? (
                  ...
                  <>
                    <div className="px-05 py-03 border-b border-border-subtle-01 cds-helper-01 text-text-secondary sticky top-0 bg-layer-02">
                      {filePath} · {fmtBytes(preview.size)}
                      {preview.truncated ? ' · 앞 256KB만 표시' : ''}
                    </div>
                  ...
                  <div className="px-06 py-08 cds-body-compact-01 text-text-secondary">
                    바이너리 파일입니다 ({fmtBytes(preview.size)}) — 내용 미리보기는 텍스트 파일만 지원합니다.
                  </div>
