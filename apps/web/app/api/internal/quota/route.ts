...
import { getQuotaStatusBySlot } from '@/lib/services/examOpsService';

// POST /api/internal/quota — LiteLLM 중계가 호출하는 쿼터 조회 엔드포인트. 근거: docs/batch-prompt-quota T4.
...
//
// route는 얇게: secret 검증 → body 검증 → 쿼터 조회 → 봉투 반환.
// 훅 측은 조회 실패 시 fail-open(시험 중 멈춤 방지). 이 경로 자체는 정상 처리.
...
export async function POST(req: Request) {
  ...
  try {
    status = await getQuotaStatusBySlot(batchId.trim(), slotNo as number);
  } catch (e) {
    const msg = e instanceof Error ? e.message : '쿼터 조회 중 오류가 발생했습니다.';
