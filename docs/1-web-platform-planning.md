...
> 개정 이력: v2 — Delivery/코어 분리 1급 원칙화. v3 — 역할 단순화·채점 모듈 분리·스크립트 외부화.
> **v4 — codex 리뷰 반영: attempt에서 채점상태 제거, org 역할 스코프, accepted submission + 무결성, 불변 problem version,
...
채점·리포트는 이 seam 바깥의 교체·후속 모듈이라, 바뀌어도 스키마·역할·대시보드·웹 기획은 그대로.**
...
직무별(출발: **기획**) 과제를 AI 협업으로 풀고, **과정(AI 채팅) + 결과(산출물)**를 함께 채점해 리포트로 제공.

- **차별점:** 비개발 직무도 바이브코딩 툴로 자기 업무를 자동화/에이전트화 → 취업·생산성에 활용.
- **평가 2기둥(예시 리포트):** `AI 채팅 평가 50` + `결과(산출물) 평가 50`. 결과의 객관성이 가격·신뢰의 척추.
- **이미 2개 대학 계약, 9월 오픈.** 동시 응시 **최대 50명/회차(벌크)**.
- ⭐ **지금 범위 = 뼈대.** 채점·평가 리포트는 **별도 모듈**(자체 `grading` schema·기능·페이지). 코어는 `accepted된
  submission`이라는 **입구**까지만. 그 위(채점·점수·리포트)는 나중에 붙인다.
- ⭐ **제공 방식은 코어 바깥:** 코어는 submission만 소비한다. attempt는 `delivery_mode` *셀렉터*만 가질 뿐, submission
  하류(채점·리포트·대시보드)는 제공 방식을 모른다. 호스팅이 막히면 BYOD로 내려도 하류 무변경. (→ §5)
...
셸이 다른 영역을 route group으로 분리한다. **뼈대 화면은 "진행·제출 현황" 중심이고, 점수·리포트 화면은 채점 모듈 소관.**
...
└── (app)/                       # 로그인 후. 영속 대시보드 셸 + 역할필터 메뉴
    ...
    │   └── reports/[id]/page.tsx#   [채점 모듈 소관] 뼈대엔 placeholder
    ├── org/                     # ── org_admin(고객, 자기 대학만)
    │   ├── dashboard/page.tsx   #   우리 대학 응시·제출 현황(점수집계는 채점 모듈 이후)
    │   ├── students/page.tsx    #   우리 대학 학생·제출 현황(DataTable)
    │   ├── students/[id]/page.tsx#   개별 진행/제출 (점수·리포트는 채점 모듈)
    ...
    └── admin/                   # ── admin (사내: 개발자·기획자·영업, 풀권한)
        ...
        └── submissions/page.tsx #   제출물 수집·검증 현황(채점 결과는 채점 모듈)
```
...
- [ ] ⭐ 뼈대 화면은 **점수/리포트를 모른다**(진행·제출 현황만). 점수·리포트 route는 채점 모듈이 소유
...
| `author`,`grader` | 출제/채점 | (별도 채점 모듈) | 역할만 예약 |
...
= 코어. **`hosted.slots`** = 호스팅 어댑터 전용(별도 schema). **채점·리포트(`grading`)는 별도 모듈** — 코어엔 `accepted된
submission`이라는 입구만.
...
CREATE TABLE exam.attempts (
  ...
  status TEXT NOT NULL DEFAULT 'ready'                -- ⭐ 채점상태 없음(grading 모듈 소관)
...
- 채점·리포트(`grading.*`)는 **`accepted` submission만 입력**으로 02 §10·§12에 준해 나중에 독립 제작.
...
로그인 → my/exams → [시작](starts_at·deadline_at 서버설정) → exam/[id]/intro(규칙·환경점검)
 ...
 → 서버가 deadline 재판정 → submission(received→validating→accepted/rejected) → done → (채점 모듈 후) reports
```
...
- [ ] 온보딩: 학교담당자 초대 / 벌크 학생 첫 로그인(임시비번 or 매직링크) — §아래 운영 표
...
**원칙(정확히):** 코어 attempt는 `delivery_mode` **셀렉터**를 가진다. 하지만 submission **하류**(채점·리포트·대시보드·웹)는
제공 방식을 모른다. 어댑터의 계약 = "attempt에 대해 **accepted된 정규화 Submission**(대화로그+산출물+출처+신뢰)을 만든다."

```
        ┌──────────── 코어 ────────────┐        ┌─ 후속 모듈 ─┐
정체성/역할/org · 회차/응시(+delivery 셀렉터) · submissions ─→ │ 채점·리포트  │ → 대시보드/웹
        └──────▲───────────────────────┘        └────────────┘
...
- 프록시·BYOD 스크립트 **둘 다 이 스키마**로 출력. 플랫폼은 **JSON Schema 검증 + accepted/rejected fixture + validator**를
  P0로 보유(스크립트 구현은 외부지만 **출력 계약 검증은 플랫폼 책임**). 채점 모듈은 이 한 포맷만 소비.
...
- (후속) 채점·리포트는 `submissions.trust`로 신뢰배지. BYOD는 "결과(산출물)" 객관 채점은 유효, "과정(대화)"은 신뢰 표기+휴리스틱 보강. 이 분리 없으면 BYOD 전환 시 리포트 신뢰도가 조용히 무너짐 → `trust`로 코어에 박음.
...
| 우선 | 화면 | 비고 |
|---|---|---|
| P0 | `exam/[attemptId]`(분기)+intro/done | hosted iframe / byod 업로드 → submission(accepted) |
| P0 | `login` + 역할 라우팅 | ROLE_HOME |
| P0 | `admin/batches` 개설 + 로스터 import·계정 + delivery_mode | 운영 진입점 |
| P0 | 정규화 포맷 **validator + fixture**(비-UI) | seam 무결성. BYOD/hosted 공통 입구 |
| P1 | `org/dashboard`,`org/students(/[id])` | 자기 대학 응시·제출 현황(점수는 채점 모듈 후) |
| P1 | `(marketing)` 랜딩 + `sample-report` | 영업 자료 |
| P2 | `my/exams` · `admin/orgs`,`admin/students`,`admin/submissions` | 시험목록·관리·제출 검증현황 |
| P3 | `admin/problems`(버전) · `how-it-works`·`curriculum` | 문제 관리·마케팅 보강 |
| — | 점수·`reports` 렌더 | **채점·리포트 모듈 소관**(별도). 뼈대엔 placeholder |
...
- **채점·리포트 모듈(별도 트랙, 본 문서 밖)**: `grading` schema·채점·리포트 페이지. 입력 = `accepted` submission + problem_version.
...
**Out(별도/보류):**
- **채점·평가 리포트 모듈 전체**(`grading` schema·기능·페이지). 뼈대는 `accepted submission`까지.
...
## 관통 적용 (대원칙 5 + 분리)
① 경계를 이름으로 — route group·역할·DB schema(+`hosted` 분리)·코어/어댑터/채점 3분리 ② 불변식은 DB가 —
`uq_attempt`, `submissions.attempt_id UNIQUE`, `slots.attempt_id UNIQUE`, problem_version 불변 FK, RESTRICT+soft-delete,
FK·CHECK ③ 단순함은 선택 — 사내 1역할(admin), 풀 스케일링, 채점·인프라 분리 ④ 패턴 > 도구 — 06/04/02 이식
⑤ 클라 입력은 적대적 — deadline·소유권·**trust는 서버가 판정**, hidden은 코어에 없음, 업로드는 unverified.

> ⭐ **분리 세 줄:** (1) 하류(채점·리포트·대시보드·웹)는 delivery를 모른다 — attempt의 셀렉터만 바뀜. (2) 뼈대의 끝 =
> `accepted submission(+불변 problem version + 출처/trust)`; 채점·리포트는 그걸 입력으로 하는 별도 모듈. (3) hosted 인프라는
