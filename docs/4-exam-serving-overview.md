...
앱(web)이 로그인→배정→1:1 프록시로 중계. AI는 LiteLLM 게이트웨이 1개를 공유. 장애는 pod 단위로 격리되고 최후엔 BYOD로 강등.**
...
[배포·슬롯 전체 실패]  → delivery_mode='byod' 자동 강등 → 학생은 본인 PC로 진행(시험 자체는 안 멈춤)
[시험 종료]            → 취합 완료 확인 후 replicas 50→0. PVC 잔존, 평시 자원 0
```

핵심: **한 곳이 죽어도 전체가 안 죽는다.** 그리고 호스팅이 통째로 실패해도 **BYOD라는 최후 안전망**이 있어 시험은 진행된다.
...
| **BYOD 강등** | `batches.delivery_mode='byod'` → 회차 폴백(§6) |
...
- [ ] **폴백 검증**: sync 실패/슬롯 부족 시 BYOD 자동 강등이 실제로 동작하는지 리허설
...
한 명이 죽어도 전체는 살고, 호스팅이 통째로 실패해도 BYOD로 시험은 멈추지 않는다.**
