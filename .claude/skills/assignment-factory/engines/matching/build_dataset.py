...
# 연소득_원(income_year_max 축) — 월소득과 별개 축
...
def add_member(age, addr, emp, income, owned, enroll=None, marital=None,
    ...
    """회원 1명 추가. owned=주택소유여부('예'|'아니오').
       ...
       income_year=연소득_원(None이면 풀에서 무작위).
...
def member_passes(m, p):
    ...
    if p["company_size"] and m["기업규모"] != p["company_size"]:
        return False
    if p["income_max"] is not None and (m["월소득_원"] or 0) > p["income_max"]:
        return False
    if p["income_year_max"] is not None and (m["연소득_원"] or 0) > p["income_year_max"]:
        return False
...
MEMBER_COLS = ["회원ID", "이름", "생년월일", "성별", "거주지", "취업상태", "기업규모",
               "월소득_원", "연소득_원", "최종학력", "재학상태", "혼인여부",
               "주택소유여부", "가구원수", "이메일", "가입일"]
P1COLS = ["정책ID", "정책명", "분야", "거주지요건", "연령_min", "연령_max", "취업요건",
          "기업규모요건", "소득상한_월_원", "연소득상한_원", "무주택요건", "학력요건", "혼인요건"]
...
def build_p1(path, policies, answer):
    ...
    guide = [
        ...
        ["연령_min/max", "형식: 만 나이 정수. 포스터에 하한/상한 없으면 빈칸. 예: 19 / 34"],
        ["취업요건", "허용값: '무관'·'미취업'·'재직'(미취업/재직 모두 단기근로 허용). 무관이면 빈칸. 예: 재직"],
        ["기업규모요건", "허용값: 포스터의 기업규모 문구 그대로(예: '중소기업'). 무관/언급 없으면 빈칸. 예: 중소기업"],
        ["소득상한_월_원", "형식: 월 소득 상한(원, 정수). 포스터에 없으면 빈칸 (0과 빈칸은 다름). 예: 3588020"],
        ["연소득상한_원", "형식: 연 소득 상한(원, 정수). 월소득과 별개 축. 포스터에 없으면 빈칸. 예: 75000000"],
...
def build_p2(path, matches, answer):
    ...
    guide = [
        ...
        ["나이", "회원 만 나이가 정책 연령(연령_min~연령_max, 양끝 포함) 안. "
        ...
        ["월소득", "소득상한_월_원이 있으면 회원 월소득_원이 그 이하(같으면 통과) / 빈칸=모두"],
        ["연소득", "연소득상한_원이 있으면 회원 연소득_원이 그 이하(같으면 통과). 월소득과 별개 / 빈칸=모두"],
