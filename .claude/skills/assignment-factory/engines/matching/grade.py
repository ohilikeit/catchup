...
def norm_bool(v):
    s = norm_str(v).upper()
    if s in ("Y", "예", "TRUE", "O"): return "Y"
    if s in ("N", "아니오", "FALSE", "X"): return "N"
    return s

def norm_constraint(v):
    """취업/학력/혼인 요건: '무관'·'없음'·'제한없음'·빈칸을 모두 '제약없음'으로 동일 처리."""
    s = norm_str(v)
    if s in ("", "무관", "없음", "제한없음", "해당없음"): return ""
    return s

def norm_house(v):
    """무주택요건: '필요'·'Y'·'예'·'O'·'무주택'·'필수'를 모두 '필요'로, 그 외는 빈칸."""
    s = norm_str(v).upper()
    if s in ("필요", "Y", "예", "O", "무주택", "필수", "TRUE"): return "필요"
    return ""
...
def grade_part1(sub, ans):
    A = {norm_str(r["정책ID"]): r for r in ans}
    S = {norm_str(r["정책ID"]): r for r in sub}
    ...
    for pid, arow in A.items():
        ...
        for f, fn in P1_FIELDS.items():
            ...
            if not ok:
                detail.append({"정책ID": pid, "필드": f, "정답": exp, "제출": got})
...
def grade_part2(sub, ans, valid_policies, valid_members):
    ...
    # 무결성: 추천 대상 아님(matchable=N 등) 정책 추천 / 유령 ID
    bad_policy = sorted({(m,p) for (m,p) in S if p not in valid_policies})
    ghost_id   = sorted({(m,p) for (m,p) in S if m not in valid_members})
    # 판별불가(matchable=N) 추천 탐지: 추천 대상(valid)이지만 정답 매칭이 0건인 정책
    #   = 회원 명단으로 자격을 확인할 수 없어 정답이 '아무에게도 추천 안 함'인 정책.
    #   여기에 추천을 보냈다면 오발송(무리하게 채운 판단 실패)으로 본다.
    ...
    return {
        ...
        "추천불가정책추천": bad_policy[:50], "유령ID사용": ghost_id[:50],
        "판별불가정책추천": unmatchable[:50],
        "FP샘플": sorted(fp)[:20], "FN샘플": sorted(fn)[:20],
    }

# ---- 배점 -----------------------------------------------------------------
# 날짜 필터를 제거하면서 옛 '마감필터(15)' 비중은 Part1 추출로 흡수했다.
...
def main():
        ...
        with (adir/"policies.csv").open(encoding="utf-8-sig", newline="") as _f:
            for _r in _csv.DictReader(_f):
                if (_r.get("matchable") or "").strip().upper() == "N":
                    unmatchable.add(norm_str(_r.get("policy_id")))
    except Exception:
        pass
    ...
    try:
        ...
        valid_members = {m for (m, _) in
                         {(norm_str(r.get("회원ID")), 1) for r in p2_ans}}  # fallback
    ...
    if r2["추천불가정책추천"]: print(f"        ⚠ 추천불가 정책 추천: {len(r2['추천불가정책추천'])}건")
    if r2["유령ID사용"]:     print(f"        ⚠ 존재하지 않는 회원ID: {len(r2['유령ID사용'])}건")
    if r2["판별불가정책추천"]: print(f"        ⚠ 판별불가(추천불가) 정책 추천: {len(r2['판별불가정책추천'])}건")
    print("-"*60)
