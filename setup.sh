...
# docs/5 §2 + docs/10: 단일 버킷 catchup-bucket(private). 용도별(scaffold/artifacts/chatlogs/snapshots)은
# 이 버킷 안의 최상위 폴더(키 접두)로 분리한다. 앱도 첫 put 에서 ensureBucket 하지만 미리 보장.
...
if kubectl -n "$NS" exec deploy/minio -- sh -c '
  ...
  mc mb -p "lo/catchup-bucket" >/dev/null 2>&1; mc anonymous set none "lo/catchup-bucket" >/dev/null 2>&1
  mc ls lo
' 2>/dev/null | grep -q catchup-bucket; then
  ok "버킷 catchup-bucket 준비(private) — scaffold/artifacts/chatlogs/snapshots 는 하위 폴더"
else
