...
[입력]
  eval/answer_key/policies.csv   ★ 사용자가 실제 정책 데이터로 채우는 단일 진실 소스.
                                 ...
                                 apply_start,apply_end,alt_age_max,alt_income_median_pct
                                 ...
                                 · alt_age_max / alt_income_median_pct : 복수 자격 트랙(OR 분기)용
                                   보조 컬럼. 둘 다 비면(대다수 정책) 기존과 완전히 동일하게 동작.
                                   alt_age_max가 있으면 "기본 트랙(age_min~age_max) OR 대안 트랙
                                   (나이<=alt_age_max AND 가구중위소득<=alt_income_median_pct%)"
                                   으로 연령을 판정한다 — 그 외 축(지역·취업 등)은 두 트랙 공통.
  데이터/포스터/*                실제 정책 홍보 포스터(사용자가 직접 넣음).
                                 ── 이 스크립트는 이 폴더를 절대 만들거나 지우거나 건드리지 않는다.

[출력]
  데이터/회원명단.xlsx               회원 DB (약 1,000명, 정책 axes 기반 경계 함정 자동 포함)
  제출물/최종제출.xlsx                 학생 제출 양식(빈 표 + '안내' + 예시행, 시트: 안내/타게팅/정책목록/소득기준표)
  eval/answer_key/1_정책정리표_정답.xlsx   정책정리 정답(운영자 전용, eval 내부라 영어 유지)
  eval/answer_key/2_추천리스트_정답.xlsx      타게팅 정답 ((회원ID,정책ID) 매칭 전부, 영어 유지)
...
# 주의: 데이터/포스터/ 의 실제 포스터는 사용자가 넣는다 → 코드는 손대지 않는다.
...
# 75M 초과 비중을 1/9→4/12로 올려 YP07(연소득 상한 75M)의 정답 지분을 낮춘다
# (단일 정책이 정답의 절반을 쥐면 그 정책 하나의 처리 실수가 총점을 지배 — 플레이테스트
# 실측 후 분포 조정. 정책 조건·포스터는 불변, 회원 모집단만 조정).
INCOME_YEAR_POOL = [0, 30_000_000, 98_000_000, 75_000_000,
                    78_000_000, 82_000_000, 86_000_000, 90_000_000, 95_000_000,
...
def load_policies():
    ...
    with CSV_PATH.open(encoding="utf-8-sig", newline="") as f:
        ...
        for r in reader:
            ...
            nh_raw = (r.get("no_house") or "").strip().upper()
            if nh_raw not in ("", "Y", "N"):   # 오입력(예 '필요')이 조용히 '무관'이 되는 것 방지(fail loud)
                sys.exit(f"[중단] {pid}: no_house '{nh_raw}'는 허용되지 않음(Y|N|빈칸).")
            enroll_raw = (r.get("enroll_status") or "").strip() or "무관"
            if enroll_raw not in ("무관", "재학", "졸업"):
                sys.exit(f"[중단] {pid}: enroll_status '{enroll_raw}'는 허용되지 않음(무관|재학|졸업).")
            marital_raw = (r.get("marital") or "").strip() or "무관"
            if marital_raw not in ("무관", "미혼", "신혼부부"):
                sys.exit(f"[중단] {pid}: marital '{marital_raw}'는 허용되지 않음(무관|미혼|신혼부부).")
            ...
            rows.append(dict(
                ...
                # OR 분기(복수 자격 트랙) 축 — 기본 연령 상한(age_max) 미달 시에만 참조하는
                # '대안 트랙' 상한/소득요건. 둘 다 비면(None) 기존 정책과 완전히 동일하게 동작.
                alt_age_max=int_or_none(r.get("alt_age_max")),
                alt_income_median_pct=int_or_none(r.get("alt_income_median_pct")),
                no_house=(nh_raw == "Y"),
                enroll=enroll_raw,     # 무관|재학|졸업
                marital=marital_raw,   # 무관|미혼|신혼부부
...
def plant_boundaries(p):
    ...
    # 연소득 통과값은 이 정책에 연소득 축이 **있을 때만** 고정한다. 축이 없는 정책의
    # 경계 회원까지 통과값을 주면(구버전) 그들이 연소득 축을 가진 다른 정책(YP07)에
    # 일괄 통과해 그 정책의 정답 지분을 인위적으로 키운다 — 축이 없는 정책은 이 값을
    # 아예 보지 않으므로 랜덤(None→풀)이어도 자기 경계 격리는 깨지지 않는다.
    piy = passing_income_year(p) if p["income_year_max"] is not None else None
    ...
    # alt_age_max가 있는 정책은 age_max/age_max+1 경계에서 통과 여부가 소득에도
    # 좌우되므로(대안 트랙 개입), 그 두 지점은 여기서 심지 않고 (A'') 블록에서
    # 소득을 명시적으로 통제해 심는다(모호한 경계 표본 방지).
    age_targets = []
    if p["age_min"] is not None:
        age_targets += [p["age_min"] - 1, p["age_min"]]
    if p["age_max"] is not None and p["alt_age_max"] is None:
    ...
    # (A'') 복수 자격 트랙(OR 분기) 경계 — alt_age_max가 있는 정책 전용(generic).
    #   · 기본 상한 경계(age_max/age_max+1): 대안 트랙이 끼어들지 않도록 소득을
    #     대안 중위소득 상한보다 확실히 높게 잡아, 순수하게 "기본 트랙" 판정만 관찰한다.
    #     age_max     → 기본 트랙 통과(나이 조건만으로 통과, 소득 무의미)
    #     age_max+1   → 기본 트랙 탈락 + 대안 트랙도 고소득이라 탈락 → 최종 탈락
    #   · 대안 트랙 경계(기본 상한 초과 ~ alt_age_max 이내 나이에서 가구원수 1·3의
    #     중위소득 상한/상한+1): "기본 트랙은 탈락하지만 대안 트랙으로 구제되는 회원"과
    #     "대안 트랙 소득 기준도 초과해 최종 탈락하는 회원"을 함께 심는다.
    if p["age_max"] is not None and p["alt_age_max"] is not None:
        guard_hh = 1
        guard_cap = (median_cap(guard_hh, p["alt_income_median_pct"])
                     if p["alt_income_median_pct"] is not None else base_inc)
        guard_income = guard_cap + 1_000_000   # 대안 트랙 상한을 확실히 초과시켜 무의미하게 만듦
        m(p["age_max"], matching_address(p), "아니오",
          income=guard_income, household=guard_hh)         # 기본 상한: 나이만으로 통과
        m(p["age_max"] + 1, matching_address(p), "아니오",
          income=guard_income, household=guard_hh)         # 기본 상한+1: 소득도 높아 대안 탈락 → 최종 탈락

        if p["alt_income_median_pct"] is not None and p["age_max"] + 1 <= p["alt_age_max"]:
            alt_test_age = min(p["age_max"] + 3, p["alt_age_max"])
            for hh in (1, 3):
                cap = median_cap(hh, p["alt_income_median_pct"])
                m(alt_test_age, matching_address(p), "아니오",
                  income=cap, household=hh)                # 대안트랙 구제: 기본탈락 + 대안통과 → 최종 통과
                m(alt_test_age, matching_address(p), "아니오",
                  income=cap + 1, household=hh)             # 대안 소득도 초과 → 최종 탈락
...
def synthesize_members(policies, target_count=1000):
    ...
    # 나이 17~46(청년 경계 밖 회원 포함), 월소득에 고소득 구간(중위소득 150·200% 상한
    # 초과) 추가 — 특정 정책(특히 전국·조건 넓은 정책)이 정답 지분을 독식하지 않게
    # 모집단을 넓힌다. 정책별 정답 지분은 생성 요약의 '정책별 매칭 수'로 확인.
    # 지역: 서울 가중(포털 특성) + 중랑구 별도 가중(자치구 한정 정책의 모수 확보)
    addr_makers = ([lambda: seoul_addr(random.choice(SEOUL25))] * 4
                   + [lambda: seoul_addr("중랑구")]
                   + [m for m in all_addr_makers[1:]])
    while len(_members) < target_count:
        # 나이: 일반(17~46) 2 : 청년(19~34) 1 : 문화패스 구간(21~23) 1 가중
        a = random.choice([random.randint(17, 46), random.randint(17, 46),
                           random.randint(19, 34), random.randint(19, 34)])
        add_member(
            a,
            random.choice(addr_makers)(),
            random.choice(["미취업", "미취업", "재직", "단기근로"]),
            random.choice([0, 1_200_000, 1_800_000, 2_400_000,
                           2_900_000, 3_300_000, 3_700_000, 4_500_000,
                           5_600_000, 6_900_000, 8_400_000]),
            random.choice(["아니오", "아니오", "예", "예", "예"]),   # 주택소유 60% → 무주택 요건 정책(YP03) 지분 축소
            birth=birth_random(a),   # 출생월 1~12 무작위 → 단순 빼기 함정이 모집단에 실재
        )
    return _members
...
def member_passes(m, p):
    ...
    if p["age_min"] is not None and a < p["age_min"]:
        return False
    # 복수 자격 트랙(OR 분기): alt_age_max가 있는 정책만 기본 트랙 OR 대안 트랙으로 판정.
    # alt_age_max가 None이면(대다수 정책) 기존과 완전히 동일한 단일 age_max 판정.
    if p["alt_age_max"] is not None:
        base_age_ok = p["age_max"] is None or a <= p["age_max"]
        if not base_age_ok:
            # 대안 트랙: 나이가 alt_age_max 이하이고, 가구 중위소득 alt_income_median_pct%
            # 이하(가구원수별 환산)여야 구제. 둘 중 하나라도 불충족하면 최종 탈락.
            if p["alt_income_median_pct"] is None or a > p["alt_age_max"]:
                return False
            cap = median_cap(m["가구원수"] or 1, p["alt_income_median_pct"])
            if (m["월소득(원)"] or 0) > cap:
                return False
...
def build_p1(path, policies, answer):
    ...
    guide = [
        ["항목", "설명"],
        ["행 단위", "정책 1건 (데이터/포스터/ 의 공고문 전부)"],
        ["입력", "데이터/포스터/ 의 실제 정책 포스터·공고문(PDF·이미지)"],
        ...
        ["★확인 불가 규칙", "자격 판정은 위 자격 축(거주지·연령·취업·기업규모·월소득·연소득·무주택·학력·혼인)으로만 한다. "
                          "단, 다음 세 유형(닫힌 목록)이 포스터의 신청자격에 명시된 정책은 Part2에서 아무에게도 추천하지 않는다 — "
                          "① 본인이 아닌 가족(부모·배우자)의 소득·재산이 금액 기준으로 명시 ② 특정 대출을 받은 사람만 대상 ③ 자녀 필요. "
                          "이 세 유형은 소제목·표가 아니라 지원대상·신청자격을 설명하는 문장의 주어·수식어에 서술될 수 있으니 문장 전체를 끝까지 읽어 판단한다. "
                          "'(가구) 기준 중위소득 ○○% 이하' 소득 기준은 여기에 해당하지 않는다 — 월소득_중위소득기준(%) 축으로 판정. "
                          "세 유형 외의 나머지 포스터 문구(신청 기간·전입 시기·절차·서류·수급자 여부·타 사업 참여 이력·"
                          "주택 거래금액·세금 조건 등)는 칸에 넣지도, 판정에 쓰지도 않는다(억지 숫자=오답)"],
...
def build_p2(path, matches, answer, median_pcts=(), policies=()):
    wb = openpyxl.Workbook()
    g = wb.active
    g.title = "안내"
    guide = [
        ...
        ["입력", "데이터/회원명단.xlsx + 데이터/포스터/ 의 정책 공고문에서 직접 읽은 자격요건(날짜 무관)"],
        ["회원ID", "회원명단.xlsx의 회원 식별자(M0001 …)"],
        ["추천정책ID", "자격이 맞는 정책의 정책ID"],
        ["★정책목록", "이 파일의 '정책목록' 시트에 추천정책ID와 포스터 파일명이 짝지어 있다. "
                    "각 정책의 자격 조건은 반드시 해당 포스터 파일에서 직접 읽는다. "
                    "추천정책ID는 반드시 이 목록의 ID를 그대로 쓴다."],
        ...
        ["거주지", "거주지요건이 '전국'=모든 회원 / 시도(예 서울특별시)=그 시도 거주자 / "
                  ...
                  "시·도가 생략된 주소(예: '중구 중1동', '강서구 중앙동')는 서울특별시 거주로 간주한다"],
        ...
        ["★확인 불가 정책", "자격 판정은 이 안내에 나열된 축(나이·거주지·취업·기업규모·월소득(본인 상한/중위소득%)·연소득·무주택·학력·혼인)으로만 한다. "
                          "단, 다음 세 유형의 자격 조건이 포스터의 신청자격에 명시된 정책은 회원 명단으로 확인할 수 없으므로 아무에게도 추천하지 않는다 — "
                          "① 본인이 아닌 가족(부모·배우자)의 소득이나 재산이 금액 기준으로 명시된 경우 "
                          "② 특정 대출을 받은 사람만 대상인 경우 ③ 자녀가 있어야 신청할 수 있는 경우. "
                          "이 세 유형의 조건은 소제목·표로 따로 적히지 않고 지원대상·신청자격을 설명하는 문장의 주어·수식어 안에 서술되어 "
                          "있을 수 있으므로, 해당 항목은 소제목·표만 보지 말고 문장 전체를 끝까지 읽어 확인한다. "
                          "주의: '(가구) 기준 중위소득 ○○% 이하' 형태의 소득 기준은 여기에 해당하지 않는다 — "
                          "이는 월소득(중위소득%) 축으로 판정한다(소득기준표 사용). "
                          "위 세 유형 외의 나머지 포스터 문구(신청 기간·전입 시기·신청 절차·제출 서류·수급자 여부·"
                          "다른 사업 참여 이력·기존 지원 한도·주택 거래금액·세금 관련 조건 등)는 판정에 사용하지 않는다"],
        ["★조건부 완화 조건", "일부 정책은 저소득 가구에 한해 연령 기준이 완화되는 대안 조건이 있을 수 있습니다 — "
                          "포스터 전체를 읽고 이런 조건부 완화가 있는지 확인하세요. 즉 나이 조건이 표·불릿 하나로만 "
                          "끝난다고 단정하지 말고, '단,' · '다만,' 등으로 이어지는 예외·완화 문장이 뒤에 더 있는지 "
                          "포스터 끝까지 확인합니다. 그런 조건부 완화가 있다면, 기본 조건과 완화된 조건 중 "
                          "하나라도 만족하면 그 정책의 자격을 충족한 것으로 봅니다(나머지 축은 두 조건 모두에서 공통으로 만족해야 함)."],
    ...
    # 정책목록 — 추천정책ID ↔ 포스터 파일명 대응표(학생이 ID를 발명하지 않도록 제공).
    # ★ 자격 조건·정책명·matchable 등은 절대 넣지 않는다(정답 유출 금지). 9개 정책 전부 수록.
    if policies:
        pl = wb.create_sheet("정책목록")
        pl.append(["정책ID", "포스터 파일명"])
        style_header(pl, 2)
        for p in policies:
            pl.append([p["id"], p["poster"]])
        pl.freeze_panes = "A2"
        pl.column_dimensions["A"].width = 10
        pl.column_dimensions["B"].width = 60
...
def main():
    ...
    # 소득기준표에 노출할 % 목록 — 기본 트랙(income_median_pct) + 대안 트랙
    # (alt_income_median_pct, OR 분기 정책용) 양쪽에서 쓰이는 % 를 모두 합친다.
    median_pcts = sorted(
        {p["income_median_pct"] for p in policies if p["income_median_pct"] is not None}
        | {p["alt_income_median_pct"] for p in policies if p["alt_income_median_pct"] is not None}
    )

    write_members(members)
    build_p1(ANS  / "1_정책정리표_정답.xlsx",   policies, answer=True)
    build_p2(SUBMIT / "최종제출.xlsx",          matches, answer=False, median_pcts=median_pcts,
             policies=policies)
    build_p2(ANS  / "2_추천리스트_정답.xlsx",   matches, answer=True,  median_pcts=median_pcts,
             policies=policies)
    print("[4/4] 엑셀 산출 완료: members / 제출양식(최종제출) / 정답키 2")
