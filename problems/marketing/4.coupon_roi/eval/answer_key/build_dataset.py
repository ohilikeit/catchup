...
def build_metrics(path, order, metrics, answer):
    ...
    guide = [
        ...
        ["사용건수", "그 쿠폰의 '결제완료' 주문이 몇 건인지(건수)"],
        ["매출합계", "그 쿠폰 '결제완료' 주문들의 매출액_원 을 모두 더한 값"],
        ["할인합계", "그 쿠폰 '결제완료' 주문들의 할인액_원 을 모두 더한 값"],
        ["공헌이익", "매출합계 × 0.3 (매출의 30%가 남는다고 봅니다). 원 단위로 반올림하되, "
                  "끝이 0.5면 올립니다."],
        ["순이익", "공헌이익 − 할인합계 (할인이 더 크면 마이너스가 됩니다)"],
        ["ROI", "순이익 ÷ 할인합계 × 100. 소수 첫째 자리에서 반올림(예: 33.25 → 33.3). "
               "'할인 1원당 얼마를 남겼나'를 %로 본 값이에요."],
        ["주채널", "그 쿠폰 '결제완료' 매출을 가장 많이 끌어온 유입채널 이름(데이터에 적힌 그대로)"],
        ["적자여부", "순이익이 0보다 작으면 '적자', 아니면 빈칸으로 둡니다"],
        ["★공헌이익 반올림 주의", "매출합계×0.3 이 예: 300001.5 처럼 끝이 .5면 올려서 300002. "
                          "그냥 버리면 1원 차이로 틀립니다."],
        ["★할인이 0인 쿠폰", "할인을 한 푼도 안 준 쿠폰(예: 무료배송)은 ROI 를 0.0 으로 적어요"
                         "(0으로는 나눌 수 없으니까). 적자 아님."],
        ["★아무도 안 쓴 쿠폰", "'결제완료'가 0건인 쿠폰은 모든 숫자를 0, ROI 는 0.0, "
                         "주채널·적자여부는 빈칸으로 둡니다."],
        ["★숫자는 딱 맞아야 해요", "사용건수·매출합계·할인합계·공헌이익·순이익은 정확히 일치해야 "
                          "정답입니다(어림·반올림 금지). ROI 만 ±0.1 까지 봐줍니다."],
        ["줄 순서", "점수와 상관없어요(쿠폰코드로 맞춰서 채점합니다)"],
    ]
    for r in guide:
        g.append(r)
...
def build_summary(path, summary, answer):
    ...
    guide = [
        ...
        ["★전체ROI (주의)", "총순이익 ÷ 총할인 × 100 으로 계산합니다. 쿠폰별 ROI 를 그냥 평균 내면 "
                       "안 돼요 — 할인을 많이 쓴 쿠폰이 더 크게 반영돼야 맞습니다. 소수 첫째 자리에서 반올림."],
        ["적자쿠폰수", "순이익이 0보다 작은(적자인) 쿠폰이 몇 개인지 센 값"],
        ["★숫자 정확도", "ROI 는 ±0.1 까지 봐주고, 합계와 개수는 정확히 맞아야 정답입니다."],
        ["★자주 하는 실수", "전체ROI 를 '쿠폰들의 ROI 평균'으로 내면 틀립니다. 반드시 총순이익 ÷ 총할인."],
    ]
    for r in guide:
        g.append(r)
    g.cell(1, 1).font = Font(bold=True)
