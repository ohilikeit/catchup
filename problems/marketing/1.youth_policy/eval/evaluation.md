...
| `소득상한_월_원` | 정수(원, `income_max_month`). 상한 없으면 **빈칸** | Exact |
| `연소득상한_원` | 정수(원, `income_year_max`). 상한 없으면 **빈칸** | Exact |
...
│   • 키(정책ID)로 행 정렬·조인
│   • P1_FIELDS = 거주지요건·연령_min·연령_max·취업요건·기업규모요건(norm_constraint)·
│     소득상한_월_원·연소득상한_원(norm_int)·무주택요건(norm_house)·학력요건·혼인요건
...
- `norm_int`(소득상한_월_원·연소득상한_원): 천단위 콤마 제거 후 정수 비교. 빈칸=제약없음, `0`=상한 0원으로 구분.
