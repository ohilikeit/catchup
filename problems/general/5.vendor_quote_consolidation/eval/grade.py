# -*- coding: utf-8 -*-
"""
거래처 견적 통합(취합/통합 + 보조 집계) 과제 — 채점 스크립트
...
배점: 견적비교표 90 (셀 Exact 60 + 행집합 Set-F1 30) / 무결성 10.
  채점 = Set(품목코드) + Exact(정규화 칸별). extra=중복미제거/유령, missing=누락.
  정답키는 build_dataset.py 가 생성한 eval/answer_key/견적비교표_정답.xlsx 를 쓴다.
...
# ---- 견적비교표 (Set(품목코드) + Exact(정규화 셀)) --------------------------
def grade_part1(sub, ans):
    A = {rowkey(r): r for r in ans}
    # 제출은 품목코드 중복(=같은 품목 여러 줄) 가능 → 키별 행 목록
    S_list = {}
    for r in sub:
        k = rowkey(r)
        if k:
    ...
    # 행집합 정밀도는 '제출한 데이터 행 전체'를 분모로 — 중복 미접기·유령 행이 그대로 FP가 된다.
    ...
    # 셀 Exact: 정답 키마다 (중복 제출이면 첫 행) 8칸 대조
    ...
    return {
        ...
        "유령행수": len(ghost), "중복미접기수": len(dup),
        "유령행": ghost[:50], "중복미접기": dup[:50],
...
# ---- 배점 -----------------------------------------------------------------
# 견적비교표 셀 Exact 60 / 행집합 Set-F1 30 / 무결성 10.
def score(p1):
    s_cell = p1["셀정확도"] * 60
    s_row = p1["행집합_F1"] * 30
    viol = p1["유령행수"] + p1["중복미접기수"]
    s_int = max(0, 10 - viol * 0.5)
    return {
        "견적비교표_셀Exact(60)": round(s_cell, 1),
        "견적비교표_행집합F1(30)": round(s_row, 1),
...
def main():
    ...
    sub = load_sheet(args.part1, SHEET)
    ans = load_sheet(adir / "견적비교표_정답.xlsx", SHEET)

    r1 = grade_part1(sub, ans)
    sc = score(r1)

    print("=" * 64); print(" 채점 결과 (거래처 견적 통합)")
    print("=" * 64)
    print(f"[견적비교표] 셀정확도 {r1['셀정확도']*100:.1f}% ({r1['정확셀']}/{r1['전체셀']}) | "
    ...
    print(f"        정답행 {r1['정답행수']} / 제출(고유) {r1['제출행수(고유)']} / 제출(원본) {r1['제출행수(원본)']} | "
          f"누락 {r1['누락_FN수']}")
    print(f"        필드별: {r1['필드별']}")
    if r1["유령행수"]:     print(f"        ⚠ 유령 품목코드(미존재): {r1['유령행수']}건")
    if r1["중복미접기수"]: print(f"        ⚠ 중복 미접기(같은 품목코드 다중 행): {r1['중복미접기수']}건")
    print("-" * 64)
    for k, v in sc.items():
        print(f"  {k:<24} {v}")
    print("=" * 64)
